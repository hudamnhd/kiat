...
import {
  ...
  updateTask,
  updateTasksColumn,
} from "#src/features/daily-tasks/actions";
import type { Task, TaskObject } from "#src/features/daily-tasks/actions";
...
async function load_data_daily_tasks() {
  ...
  function updateStatus(data: TodoState) {
    for (const date in data) {
      ...
      for (const task of tasks) {
        ...
        if (task.status === "progress") {
          ...
          const diffMinutes = (now.getTime() - lastSessionTime.getTime()) /
            1000 / 60;
...
const calculateGlobalSessionCount = (data: TaskObject) => {
  const result = {};
...
const calculateStreak = (data: TaskObject) => {
...
const exportTodos = (todos: TaskObject) => {
  // Mengonversi data todos ke format JSON
  const jsonData = JSON.stringify(todos);

  // Membuat Blob untuk file JSON
  const blob = new Blob([jsonData], { type: "srclication/json" });
...
const validateTodoData = (todosData: TaskObject) => {
...
const importTodos = (event, data) => {
  ...
  if (file && file.type === "srclication/json") {
...
const Layout = () => {
  ...
  if (!initial_data) {
    return <TodoNavigator data={todos} />;
  }
};

const Unload = ({ data }: { data: TaskObject }) => {
  React.useEffect(() => {
...
const TaskApp = ({
  ...
  streak_data: { current_streak: number; longest_streak: number };
  all_session: any;
  active_task: Task;
}) => {
  ...
  const calculateTotalSessions = (todos: Task[]) => {
    return todos.reduce(
      (total_sessions, todo) =>
        total_sessions +
          todo.sessions.filter(
            (d) => new Date(d) + sessionDuration < Date.now(),
          ).length || 0,
      0,
    );
  };
  ...
  const calculateTotalTime = (todos: Task[]) => {
  ...
  return (
    <div className="">
      ...
      <div className="px-2.5">
        ...
        <div
          style={{ animationDelay: `0.1s` }}
          className="animate-slide-top [animation-fill-mode:backwards] ml-auto flex w-full items-center  bg-linear-to-r from-background to-accent py-2 pr-2 mt-1 mb-2 rounded-md"
        >
      ...
      >
        <div className="relative h-8 w-full rounded-md bg-muted">
          ...
          <div
            ...
            style={{
              width: `${
                (total_sessions > 16 ? 16 / 16 : total_sessions / 16) * 100
              }%`,
            }}
          >
            <div
...
const TodoTimer = ({
      ...
      if (!timerRef.current && last_start) {
        timerRef.current = setInterval(() => {
          ...
          document.title = `${timer} ${
            active_task.title ? active_task.title.slice(0, 10) : "Untitled"
          }`;
          ...
          if (updatedTotalTime >= TWENTY_FIVE_MINUTES) {
            ...
            const notif = {
              ...
              description: "Sesion " + active_task.sessions.length +
                " has completed",
            };
    ...
    if (break_time) {
      ...
      timerRef.current = setInterval(() => {
        ...
        if (
          ...
          container_break_time instanceof HTMLDivElement
        ) {
  ...
  return (
    <div className="flex items-start justify-between gap-x-3 px-4 md:gap-x-5 h-[110px] mt-4">
          ...
          <div className="relative">
            <div className="absolute flex h-full w-full justify-center">
              <div className="flex flex-col justify-center items-center">
                {break_time ? <Coffee className="text-chart-1" /> : (
                    ...
                    <Button
                      ...
                      onPress={active_task
                        ? () => {
                          ...
                          const last_session = active_task
                            ?.sessions[last_session_index];
          ...
          >
            <div className="font-bold md:text-xl line-clamp-1 max-w-[250px]">
              {active_task?.title !== ""
                ? (
                  <div
                    dangerouslySetInnerHTML={{ __html: active_task.title }}
                  />
                )
                : (
                  "Untitled"
            ...
            <div className="mt-2 flex w-full flex-col gap-2 pr-14 md:flex-row md:items-center md:pr-0">
              <Button
                ...
                onPress={() => {
                  ...
                  const notif = {
                    title: "Saatnya istirahat",
                    description: "Sesion " + sessionData.length +
                      " has completed",
                  };
                  showNotification(notif.title, notif.description);
              ...
              <button className="hidden items-center justify-center font-medium ring-offset-background transition-colors focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 rounded-md flex h-auto gap-1 p-1 py-1 bg-transparent text-foreground hover:bg-red-600 hover:text-white">
  ...
  return (
    <div className="bg-background">
      ...
      <div className="flex gap-10">
        ...
        <div className="sm:block hidden w-full max-w-[240px] rounded bg-muted/50 p-5">
    ...
    <div className="bg-background">
      <div className="flex gap-10">
        <div>
          ...
          <div className="grid grid-cols-7 justify-center gap-5 rounded-lg bg-muted p-2 px-3 text-foreground">
            <div>Mo</div>
            <div>Tu</div>
            <div>We</div>
            <div>Th</div>
...
const focusSessions = [
  {
    ...
    styles: {
      outerBorder:
        "border-white bg-linear-to-r from-blue-500 to-blue-300 ring-4 ring-orange-400",
      middleBorder: "border-white bg-linear-to-r from-yellow-300 to-yellow-100",
      innerBorder:
        "border-green-500 bg-linear-to-r from-green-500 to-green-300",
    },
  },
  {
    minFocus: 12,
    ...
    styles: {
      outerBorder: "border-blue-500 bg-linear-to-r from-blue-500 to-blue-300",
      middleBorder: "border-white bg-linear-to-r from-yellow-300 to-yellow-100",
      innerBorder:
        "border-green-500 bg-linear-to-r from-green-500 to-green-300",
    },
  },
  {
    ...
    styles: {
      outerBorder: "border-muted-foreground",
      middleBorder: "border-white bg-linear-to-r from-yellow-300 to-yellow-100",
      innerBorder:
        "border-green-500 bg-linear-to-r from-green-500 to-green-300",
    },
  },
  {
    ...
    styles: {
      outerBorder: "border-muted-foreground",
      middleBorder: "border-muted-foreground",
      innerBorder:
        "border-green-500 bg-linear-to-r from-green-500 to-green-300",
    },
  },
  {
    ...
    styles: {
      outerBorder: "border-muted-foreground",
      middleBorder: "border-muted-foreground",
      innerBorder: "border-muted-foreground",
    },
  },
];
...
const WeeklyBadge: React.FC<WeeklyBadgeProps> = ({ total_sessions }) => {
  ...
  return (
    <div
      className={`shrink-0 relative overflow-hidden bg-linear-to-r ${color} rounded-lg w-full max-w-[250px] mx-auto shadow-lg`}
    >
      <svg
      ...
      <div className="relative pt-10 px-10 flex items-center justify-center">
        ...
        <div className="relative mb-4 ">
          <div
            className={`absolute inset-0 animate-pulse bg-linear-to-r ${color} rounded-full blur-lg opacity-75`}
          >
          </div>
          <div
            className={`relative flex items-center justify-center w-24 h-24 rounded-full bg-linear-to-b ${color} p-4 shadow-xl ring-3 ring-gray-200`}
          >
      ...
      <div className="relative text-white px-6 pb-6 mt-6 w-[200px]">
        ...
        <div className="relative w-full bg-gray-200 dark:bg-gray-700 h-4 rounded-full overflow-hidden mt-4">
          <div
            className="bg-linear-to-r from-green-400 to-green-600 h-full rounded-full"
  ...
  ({
    ...
    return (
      <Collapsible
        ...
        defaultExpanded={active_task?.id === task?.id ? true : false}
      >
        <div
          draggable
          ...
          className={cn(
            ...
            active_task && todo.status !== "progress"
              ? "text-muted-foreground bg-muted blur-xs transition-all duration-300"
              : active_task && todo.status === "progress"
          ...
          >
            <div
              className={cn(
                ...
                active_task && todo.status === "progress"
                  ? " bg-linear-to-r from-accent to-muted"
                  : " bg-linear-to-l from-background to-secondary",
                todo.status === "completed" &&
                  " bg-linear-to-r from-background to-chart-2/40",
              )}
            >
              <div className="flex items-center gap-x-2">
                    ...
                    <button
                      onClick={() => {
                        ...
                        toast({
                          ...
                          description:
                            `The task has been successfully added in today.`,
                        });
                      }}
        ...
        <div className="w-full">
          <div className="relative flex w-full items-start gap-2">
            ...
            <AutosizeTextarea
              ...
              style={{ resize: "none" }}
              className="w-full bg-transparent py-0 px-1 outline-hidden text-md border-none focus-visible:ring-offset-0 focus-visible:ring-0 outline-hidden"
              maxHeight={800}
              minHeight={20}
              placeholder="Untitled"
              autoComplete="off"
...
const TimelineInfo: React.FC<{ sessions: any[] }> = ({
  ...
  function adjustSessions(sessions: string[], completedAt: string): string[] {
    ...
    for (let i = 0; i < sessions.length; i++) {
      ...
      const startStr = `${startTime.getHours().toString().padStart(2, "0")}:${
      ...
      if (i === sessions.length - 1) {
        if (endTime > completedAtDate) {
          ...
          endStr = `${completedAtDate.getHours().toString().padStart(2, "0")}:${
            completedAtDate.getMinutes().toString().padStart(2, "0")
          }`;
        } else {
          // Otherwise, keep the normal end time
          endStr = `${endTime.getHours().toString().padStart(2, "0")}:${
...
const DragDropList = ({
  ...
  const [items, setItems] = useState<Task[]>(_tasks);
  ...
  const handleDragStart = useCallback(
    (event: React.DragEvent<HTMLDivElement>, index: number) => {
      ...
      dragImage.srcendChild(element);
      document.body.srcendChild(dragImage);
