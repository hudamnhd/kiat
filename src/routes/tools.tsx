import { toolsLinks } from "#src/constants/nav-link";
...
export default function Example() {
	...
	return (
		<div className="border-x min-h-[calc(100vh)] max-w-xl mx-auto pb-2">
			...
			<div role="list" className="grid grid-cols-1 gap-2 p-2.5 sm:p-3">
				{data.map((action, actionIdx) => (
					<Link
						to={action.href}
						key={actionIdx}
						className="col-span-1 flex shadow-sm rounded-md border divide-x group hover:ring-2 shadow-inner hover:shadow-xl ring-primary duration-300"
						title={action.title}
					>
						<div className="flex-shrink-0 flex items-center justify-center w-14 text-sm font-medium rounded-l-md bg-primary">
							<action.icon
								className="h-5 w-5 text-primary-foreground  sm:group-hover:scale-[140%] group-hover:-rotate-45 sm:duration-300"
								aria-hidden="true"
							/>
						</div>
						<div className="flex-1 flex items-center justify-between  rounded-r-md truncate">
							<div className="flex-1 px-4 py-1.5 text-sm truncate">
